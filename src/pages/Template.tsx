...
function Template() {
    ...
    useEffect(() => {
        const state = location.state as { selectedTicker?: string } | null;
        const newTicker = state?.selectedTicker;

        if (newTicker && newTicker !== tickerState) {
            setTickerState(newTicker);
            console.log("Ticker updated from navigation state:", newTicker);
        }
    }, [location]); 
    ...
    const handleTickerChange = (event: any, newValue: string | null) => {
        ...
        console.log("Ticker changed via Autocomplete:", newValue);
    };
